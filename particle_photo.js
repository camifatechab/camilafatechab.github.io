...
function Particle_photo(x, y) {
  ...
  this.isKilled = false;
  this.mouseSizeSlider = 150;
  this.particleSizeSlider = 20;
  this.speedSlider = 0.5;
  
  this.maxSpeed = 4; // How fast it can move per frame.
  ...
  this.move = function() {
    ...
    if (this.distToTarget > 1) {
      ...
      steer.sub(this.pos);
      steer.normalize();
      steer.mult(this.maxSpeed*proximityMult*this.speedSlider);
      this.acc.add(steer);
    }
    ...
    // Interact with mouse.
    if (mouseDist < this.mouseSizeSlider) {
      if (mouseIsPressed) {
        // Push towards mouse.
      ...
      push.normalize();
      push.mult((this.mouseSizeSlider-mouseDist)*0.05);
      this.acc.add(push);
    }
    
    // Move it.
    this.vel.add(this.acc);
    this.vel.limit(this.maxForce*this.speedSlider);
